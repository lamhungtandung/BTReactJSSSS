...
export default class MainComponent extends Component {
    ...
    //nhập
    inputSeat = () => {
        let { userName, seatNumber } = this.state;
        //kiem tra
        if (userName === '' || seatNumber === 0) {
    ...
    checkItem = (value) => {
        for (let i = 0; i < this.state.arrSeat.length; i++) {

            if (value === this.state.arrSeat[i].soGhe) {
                return '#ff9800'
            }
    ...
    addSeat = (seat) => {
        ...
        if (this.state.warning === 'block') {
            let { seatNumber, arrSeat } = this.state;
            ...
            if (arrSeat.length < seatNumber) {
                ...
                let tempArr = arrSeat.splice(0, seatNumber);
                this.setState({
                    arrSeat: tempArr
                })
            }
                ...
                <div className="body__content">
                    <div id='content__form' className="card container mx-auto my-5">
                        <div className="card-body">
                            <FormBook inputSeat={this.inputSeat} handleChange={this.handleChange} />
                            <ListBook warning={this.state.warning} ListBook={this.ListBook} addSeat={this.addSeat} checkItem={this.checkItem} okSeat={this.okSeat} />
                            <TableBooks userName={this.state.userName} seatNumber={this.state.seatNumber} arrSeat={this.state.arrSeat} approve={this.state.approve} totalPrice={this.totalPrice} />
                        </div>
                    </div>
                </div>
            </div>
